    ...
    try:
        ...
        if os.path.exists(current_file):
            ...
            for pick in snapshot['picks']:
                ...
                import math
                if isinstance(price_at_rec, float) and math.isnan(price_at_rec): price_at_rec = 0
                if isinstance(current_price, float) and math.isnan(current_price): current_price = price_at_rec
                if price_at_rec > 0:
                    change_pct = ((current_price / price_at_rec) - 1) * 100
                else:
                    change_pct = 0
                if isinstance(change_pct, float) and math.isnan(change_pct): change_pct = 0
...
def get_us_macro_analysis():
    try:
        ...
        macro_tickers = {
            ...
            'BTC': 'BTC-USD', '10Y_Yield': '^TNX', 'SPY': 'SPY', 'QQQ': 'QQQ',
            'USD/KRW': 'KRW=X'
        }
        ...
        for name, ticker in {'VIX': '^VIX', 'SPY': 'SPY', 'QQQ': 'QQQ', 'BTC': 'BTC-USD', 'GOLD': 'GC=F'}.items():
            try:
                stock = yf.Ticker(ticker)
                hist = stock.history(period='5d')
                if not hist.empty and len(hist) >= 2:
...
def get_us_calendar():
    ...
    try:
        ...
        if not os.path.exists(calendar_path):
            return jsonify({'events': [], 'message': 'Calendar data not available. Run economic calendar generator first.'}), 404
...
def get_technical_indicators(ticker):
    ...
    try:
        ...
        supports, resistances = detect_support_resistance(df, window=10, cluster_pct=0.02)
